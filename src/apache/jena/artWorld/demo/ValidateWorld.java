...
public class ValidateWorld extends ArtistBase
{
    /***********************************/
    /* Constants                       */
    /***********************************/
    ...
    public static void main( String[] args ) {
        new ValidateWorld().setArgs( args ).run();
    }
    ...
    public void run() {
        ...
        RDFDataMgr.read( m, WORLD_DATA_FILE );
        RDFDataMgr.read( m, CITY_DATA_FILE );
        RDFDataMgr.read( m, CITY2_DATA_FILE );
        RDFDataMgr.read( m, COUNTRY_DATA_FILE );
        RDFDataMgr.read( m, ARTIST_SCHEMA_FILE );

        // generate some output
        System.out.println("------ ArtWorld local files on Cities and Countries ---------");
        showModelSize( m );
        // SHACL
        Graph shapesGraph = RDFDataMgr.loadGraph(WorldShapes);
